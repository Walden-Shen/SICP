...
(define (make-leaf-set pairs)
 (if (null? pairs) '() (let ((pair (car pairs))) (adjoin-set (make-leaf (car pair) (cadr pair)) (make-leaf-set (cdr pairs))))))
;2.67 adabbca
(define sample-tree 
...
(define (in? symbol list)
 (cond 	((null? list) #f)
  		((eq? symbol (car list)) #t)
		(else (in? symbol (cdr list)))))
...
(define (successive-merge ordered-pairs)
 (define (anx ordered-pairs previous-tree)
  (if (null? ordered-pairs) previous-tree 
   (anx (cdr ordered-pairs) (make-code-tree (car ordered-pairs) previous-tree))))
 (anx (cdr ordered-pairs) (car ordered-pairs)))
;2.70 theoretical
;2.71 1 n-1
;2.72 n ~ n^2
